...
if(!empty($_POST["tributoEscolhido_tribut"]) && !empty($_POST["tributoEscolhido_tribut"])){
  ...
  if ($_POST["modalidade_folhaEscolhido"] == "Sem Movimento") {
    $total = 30;
  }else{
    if($_POST["modalidade_folhaEscolhido"] == "Eventos Fixos"){
      if(1 <= $_POST['qtd_funcionarioEscolhido']){
        $total += $_POST['qtd_funcionarioEscolhido'] * 15;
      }else{
        $total = 30;
      }
    }else{
...
if($personalizar == true){
  ...
  echo "<div class='card mb-4'><div class='card-body '><h5>Total da Tributário / Fiscal:</h5><h1>Personalizar</h1></div></div>";
  ...
  echo "<input type='hidden' id='total_bckoff' value='".number_format($total_contabilidade+$total_tributario+$total_dp, 2, ',','.')."'</input>";
}
?>
